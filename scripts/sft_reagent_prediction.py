"""stage4_sft_lora_reagent.py
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Stage-4: Supervised fine-tuning (SFT) for the **reagent-prediction** task.

Key differences from the retrosynthesis / forward versions
----------------------------------------------------------
1. **Dataset / collator** :  MolVQReagentDataset, collate_fn_reagent  (see MolVQReagentDataset.py)
2. **Default data path**  :  reagent-prediction json file
3. **Evaluator**          :  evaluate_reagent_prediction (character-level metrics + fingerprints)

Everything else (LoRA config, optimisation, logging) is kept identical so
that the three SFT variants can coexist in the same codebase.
"""
...
from atomdisc.evaluation.sft_evaluator_smiles import (
    evaluate_reaction_prediction as evaluate_prediction,
    log_sample_generation,
)

from MolVQDataset_reagent_prediction import (
    MolVQReagentDataset as MolVQDataset,
    collate_fn_reagent as user_collate_fn,
)
...
def sft_lora(args):
    ...
    logger = get_logger("Stage4_SFT_LoRA_Reagent", os.path.join(args.output_dir, "stage4_sft_lora.log"))
    ...
    if os.path.isdir(args.stage2_model_path):
        ...
        if args.use_structure_token:
            ...
            special = ["<mol>", "</mol>"] + [f"<atom_{i}>" for i in range(args.vq_codebook_size)]
            tokenizer.add_special_tokens({"additional_special_tokens": special})
            base_llm.resize_token_embeddings(len(tokenizer))
            logger.info(f"Added {len(special)} special tokens. New vocab = {len(tokenizer)}")
        else:
            tokenizer = AutoTokenizer.from_pretrained(args.base_tokenizer_path, use_fast=True)
            base_llm = LlamaForCausalLM.from_pretrained(args.base_llm_model_path, torch_dtype=torch.bfloat16).to(device)
            logger.info(f"No special tokens added. New vocab = {len(tokenizer)}")
    ...
    logger.info("Reagent-prediction SFT complete.")

# -------------------------------------------------------------------------
# argparser
# -------------------------------------------------------------------------
if __name__ == "__main__":
    p = argparse.ArgumentParser("Stage-4 reagent-prediction SFT (LoRA)")
    ...
    p.add_argument("--output_dir", default="./stage4_sft_reagent")
    ...
    p.add_argument("--save_every_epochs", type=int, default=100)

    # generation params
    p.add_argument("--max_new_tokens_eval", type=int, default=2048)
    p.add_argument("--num_beams_eval", type=int, default=10)
