...
# Use the same tokenizer regex as in pre-training
...
def _tokenize_smiles_string(smi: str) -> List[str]:
    ...
    while cur < len(smi):
        ...
        if m:
            ...
            # Should not happen; include the character directly for robustness
...
def convert_text_smiles_to_mol_tokens(text: str, gnn, vq, device):
    ...
    Replace each <smiles>SMI</smiles> segment with
       ...
       - Structural symbols (= # ( ) … digits) remain unchanged
        ...
        if mol:
            ...
            if graph and graph[0].nelement() > 0:
                ...
                try:
                    ...
                    canon_smi = raw_smi  # If canonicalization fails, use the raw string
